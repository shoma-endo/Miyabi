...
export class TaskOrchestrator {
     ...
     */
    getAvailableTasks(_workerId, workerSkills) {
        return this.taskQueue.filter(task => {
            ...
            if (!this.areDependenciesMet(task))
                return false;
            // Check skill match
            if (!this.hasRequiredSkills(workerSkills, task.requiredSkills))
                return false;
            return true;
        });
    }
     ...
     */
    async claimTask(workerId, taskId) {
    ...
    getWorkerTasks(workerId) {
        return Array.from(this.tasks.values()).filter(task => task.assignedTo === workerId && task.status !== 'completed' && task.status !== 'failed');
    }
    /**
    ...
    detectFileConflicts(task) {
        ...
        for (const otherTask of this.tasks.values()) {
            ...
            if (otherTask.status !== 'claimed' && otherTask.status !== 'in_progress')
                continue;
            // Check for overlapping files
            const overlap = task.metadata.files.some(file => otherTask.metadata.files.includes(file));
            if (overlap) {
