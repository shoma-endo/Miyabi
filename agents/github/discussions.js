...
export class DiscussionsClient {
    ...
    `;
        try {
            ...
            if (error.message?.includes('Discussions are disabled')) {
                throw new Error('GitHub Discussions are not enabled for this repository.\n' +
     ...
     * Search discussions
     */
    async searchDiscussions(query, first = 10) {
        const searchQuery = `
    ...
    `;
        const result = await this.graphqlClient(searchQuery, {
            owner: this.config.owner,
            repo: this.config.repo,
            query,
            first,
     ...
     */
    async getDiscussionsByCategory(categoryName, first = 10) {
        if (!this.repositoryId) {
            await this.initialize();
        }
...
`;
     ...
     */
    async createAgentQuestion(title, question) {
...
`;
     ...
     */
    async shareSuccessStory(title, story, metrics) {
