...
export async function setup(options: SetupOptions = {}) {
  ...
  // Step 1: Welcome
  const { readyToStart } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'readyToStart',
  ...
  const { hasGitHubAccount } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'hasGitHubAccount',
      message: 'GitHubアカウントをお持ちですか？',
      default: true,
    },
  ]);
    ...
    const { shouldOpenBrowser } = await inquirer.prompt([
      {
        ...
        message: 'ブラウザで https://github.com を開きますか？',
        default: true,
      },
    ]);
