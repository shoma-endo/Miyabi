...
async function main() {
    ...
    logger.newline();
    logger.box(`Claude Codeは、AIがコードを書いてくれる強力なツールです。\n\n特徴:\n• コードの自動生成\n• バグの自動修正\n• ドキュメントの自動作成\n• テストの自動実行\n\nあなたは「何をしたいか」を伝えるだけ！\nClaude Codeが自動的に作業を進めます。`, {
        title: '🤖 Claude Code',
        borderStyle: 'round',
...
`;
        const issueUrl = execSync(`gh issue create --title "${taskTitle}" --body "${issueBody.replace(/"/g, '\\"')}"`, { encoding: 'utf-8' }).trim();
        ...
        if (tutorial.toLowerCase() === 'yes' || tutorial.toLowerCase() === 'y') {
            ...
            logger.newline();
            logger.info('Claude Code Task Toolが自動的にAgentを起動し、作業を開始します。');
