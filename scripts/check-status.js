 ...
 * - Performance metrics
 */
import GitHubProjectAPI from './github-project-api.js';
...
class AgentStatusChecker {
    ...
    async getAgentStatuses() {
        ...
        const statuses = metrics.map(metric => {
            ...
            const agentItems = items.filter(item => this.getFieldValue(item, 'Agent') === metric.agent);
            const completedItems = agentItems.filter(item => item.content.state === 'CLOSED' || item.content.state === 'MERGED');
            const successRate = agentItems.length > 0
                ? (completedItems.length / agentItems.length) * 100
                : 0;
            // Determine current status
            const runningItems = agentItems.filter(item => item.content.state === 'OPEN' &&
                this.getFieldValue(item, 'Status') === 'in-progress');
            const status = runningItems.length > 0 ? 'running' :
    ...
    async displayStatus() {
        ...
        for (const agent of statuses) {
            ...
            const statusText = chalk[this.getStatusColor(agent.status)](`${statusIcon} ${agent.status.toUpperCase()}`);
    ...
    formatDuration(ms) {
        ...
        if (ms < 60000)
            return chalk.green(`${(ms / 1000).toFixed(1)}s`);
        if (ms < 3600000)
            return chalk.yellow(`${(ms / 60000).toFixed(1)}m`);
