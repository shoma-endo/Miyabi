...
export class DocGenerator {
    ...
    extractFunctionDoc(node) {
        ...
        const parameters = node.parameters.map(param => {
            ...
            const paramTag = jsDocTags.find(tag => tag.tagName.text === 'param' &&
        ...
        const examples = exampleTags.map(tag => typeof tag.comment === 'string' ? tag.comment : '').filter(ex => ex);
     ...
     */
    async generateMarkdown(docs, outputPath) {
        ...
        const interfaces = docs.filter(d => 'properties' in d && !('methods' in d));
        // Functions
     ...
     */
    generateInterfaceMarkdown(iface) {
        let md = `### ${iface.name}\n\n`;
     ...
     */
    async generateDocsForDirectory(dirPath, outputPath, recursive = true) {
     ...
     */
    getTypeScriptFiles(dirPath, recursive) {
        const files = [];
     ...
     * @param outputPath Output markdown file path
     */
    watchAndGenerate(dirPath, outputPath) {
