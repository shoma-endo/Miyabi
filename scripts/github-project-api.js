...
export class GitHubProjectAPI {
     ...
     */
    async setCustomField(itemId, fieldId, value) {
        const mutation = `
    ...
    async calculateAgentMetrics() {
        ...
        for (const item of items) {
            ...
            const duration = this.getFieldValue(item, 'Duration');
            const cost = this.getFieldValue(item, 'Cost');
            const quality = this.getFieldValue(item, 'Quality Score');
            if (!agent || agent === 'N/A')
                continue;
            if (!agentData[agent]) {
                agentData[agent] = {
    ...
    async generateWeeklyReport() {
        ...
        const completedIssues = items.filter((item) => item.content.state === 'CLOSED' || item.content.state === 'MERGED');
        const topQualityIssues = items
            .map((item) => ({
            number: item.content.number,
            title: item.content.title,
            score: this.getFieldValue(item, 'Quality Score') || 0,
        }))
            .filter((item) => item.score > 0)
            .sort((a, b) => b.score - a.score)
            .slice(0, 5);
        const totalCost = agentMetrics.reduce((sum, metric) => sum + metric.totalCost, 0);
        return {
...
//# sourceMappingURL=github-project-api.js.map