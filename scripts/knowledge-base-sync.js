...
export class KnowledgeBaseSync {
     ...
     */
    async postIssueSummary(issueNumber) {
     ...
     */
    async postWeeklyLearnings(startDate, endDate) {
     ...
     */
    async postTechnicalDecision(title, context, decision, consequences, alternatives, relatedIssues) {
...
*Recorded: ${new Date().toISOString().split('T')[0]}*`;
    ...
    extractTags(issue) {
        ...
        if (labels.some((l) => l.includes('typescript')))
            tags.push('typescript');
        if (labels.some((l) => l.includes('github')))
            tags.push('github-api');
        if (labels.some((l) => l.includes('agent')))
            tags.push('agents');
        // Extract type tags
    ...
    // Utilities
    // ============================================================================
    async findRelatedPRs(issueNumber) {
        ...
        return pulls.filter((pr) => pr.body?.includes(`#${issueNumber}`) || pr.title.includes(`#${issueNumber}`));
    }
    async getCompletedIssues(startDate, endDate) {
...
async function main() {
    ...
    switch (command) {
        ...
        case 'post-tdr':
            await kb.initialize();
            await kb.postTechnicalDecision(process.argv[3], process.argv[4] || 'Context not provided', process.argv[5] || 'Decision not provided', process.argv[6] || 'Consequences not provided', process.argv.slice(7).map((alt) => alt), []);
            break;
        default:
            console.log('Usage:');
...
//# sourceMappingURL=knowledge-base-sync.js.map