...
async function main() {
    ...
    logger.newline();
    logger.box(`Agentic OSをセットアップします。\n\nこのスクリプトは:\n• 新規プロジェクトのテンプレート作成\n• 既存プロジェクトへの途中導入\n\n両方に対応しています。`, {
        title: '✨ セットアップウィザード',
        borderStyle: 'round',
    ...
    if (scenario === 'new') {
        ...
        const useCurrentDir = await ask(`現在のディレクトリ (${currentDirName}) に導入しますか？ (yes/no): `);
...
async function setupNewProject(config) {
...
function generateCustomFiles(config) {
    // 1. BUDGET.yml
...
`;
    writeFileSync(path.join(config.projectPath, '.github', 'GUARDIAN.md'), guardianMd);
...
function mergePackageJson(config) {
...
//# sourceMappingURL=setup-agentic-os.js.map