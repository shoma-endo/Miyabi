...
async function checkTokenScopes(token) {
    try {
        const result = execSync(`curl -sI -H "Authorization: token ${token}" https://api.github.com/user`, { encoding: 'utf-8' });
        ...
        const scopes = scopeLine
            ...
            .map(s => s.trim())
            .filter(s => s);
        const hasRequired = REQUIRED_SCOPES.every(req => scopes.some(scope => scope === req.name || scope.startsWith(req.name + ':')));
        return { scopes, valid: hasRequired };
    }
    catch (error) {
        logger.error('Token の検証に失敗しました', error);
        return { scopes: [], valid: false };
    }
}
function getEnvPath() {
...
//# sourceMappingURL=setup-github-token.js.map