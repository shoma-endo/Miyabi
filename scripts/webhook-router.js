...
class WebhookEventRouter {
     ...
     */
    async triggerAgentWithRetry(agent, payload, action) {
        let lastError;
        for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
            try {
                ...
                if (attempt < RETRY_CONFIG.maxRetries) {
                    const delay = Math.min(RETRY_CONFIG.initialDelayMs * Math.pow(RETRY_CONFIG.backoffMultiplier, attempt), RETRY_CONFIG.maxDelayMs);
     ...
     */
    async triggerAgent(agent, payload, action) {
     ...
     */
    async createRoutingComment(issueNumber, agent, action) {
    ...
    if (eventType === 'issue' || eventType === 'pr' || eventType === 'comment') {
        ...
        if (labelsJson) {
            try {
                ...
                payload.labels = labels.map((l) => l.name);
            }
            catch (error) {
